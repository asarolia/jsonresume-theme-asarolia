...
function render(resumeObject) {
    ...
    if (resumeObject.certifications && resumeObject.certifications.length) {
        if (resumeObject.certifications[0].institution) {
            resumeObject.certificationsBool = true;
            _.each(resumeObject.certifications, function(e){
                if( !e.area || !e.studyType ){
                  e.certificationsDetail = (e.area == null ? '' : e.area) + (e.studyType == null ? '' : e.studyType);
                } else {
                  e.certificationsDetail = e.area + ", "+ e.studyType;
                }
                if (e.startDate) {
                    e.startDateYear = e.startDate.substr(0,4);
                    e.startDateMonth = getMonth(e.startDate || "");
    ...
    if (resumeObject.interests && resumeObject.interests.length) {
        ...
        // }
        if (resumeObject.interests[0].keywords) {
            resumeObject.interestsBool = true;
        }
    }
